...
public class DeviceUtils {
    ...
    public static int getDefaultLoadFactor(Context context) {
        if (_loadFactor == null) {
            Integer integer =
                Integer.valueOf(0xf & context.getResources().getConfiguration().screenLayout);
            _loadFactor = integer;
    ...
    public static void hideAnimatedView(View view) {
        if (PRE_HC && view != null) view.setPadding(view.getWidth(), 0, 0, 0);
    }
    ...
    public static boolean isPortrait(Context context) {
        boolean flag = true;
        if (context.getResources().getConfiguration().orientation != 1) flag = false;
        return flag;
    }
     ...
     * 获取大版本号  ex:3.8.0 -> 3.8
     */
    public static String getBigVersionName(Context context) {
    ...
    public static String getCurrentWiFiSsid(Context context) {
        String currentSsid = "";
        try {
            if (DeviceUtils.getNetworkType(context) != NETTYPE_WIFI) {
                return currentSsid;
            }
            WifiManager wifiManager = (WifiManager) context.getApplicationContext()
                .getSystemService(Service.WIFI_SERVICE);
            WifiInfo curWifiInfo = wifiManager.getConnectionInfo();
            currentSsid = curWifiInfo.getSSID();
    ...
    public static String getCurrentWiFiIp(Context context) {
        ...
        try {
            ...
            if (null != dhcpInfo) {
                currentIp =
                    (dhcpInfo.ipAddress & 0xFF) + "." + (0xFF & dhcpInfo.ipAddress >> 8) + "." + (
                        0xFF
                            & dhcpInfo.ipAddress >> 16) + "." + (0xFF & dhcpInfo.ipAddress >> 24);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static int getCellularOperatorType(Context context) {
        ...
        if ("46001".equals(operator) || "46006".equals(operator) || "46009".equals(operator)|| "46010".equals(operator)) {
            opeType = 2;
            // 中国移动
        } else if ("46000".equals(operator) || "46002".equals(operator) || "46004".equals(operator) || "46007".equals(operator)|| "46008".equals(operator)|| "46013".equals(operator)) {
            opeType = 1;
            // 中国电信
